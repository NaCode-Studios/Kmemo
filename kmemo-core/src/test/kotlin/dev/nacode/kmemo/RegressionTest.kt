...
import dev.nacode.kmemo.fixtures.HashingEmbedder
import dev.nacode.kmemo.fixtures.MutableClock
import dev.nacode.kmemo.guard.DirectionGuard
import dev.nacode.kmemo.guard.EntityGuard
import dev.nacode.kmemo.guard.GuardVerdict
import dev.nacode.kmemo.guard.LexicalDivergenceGuard
import dev.nacode.kmemo.guard.MatchGuard
import dev.nacode.kmemo.guard.MatchGuards
import dev.nacode.kmemo.guard.NumericGuard
import dev.nacode.kmemo.guard.UnitGuard
...
class RegressionTest {
    ...
    fun `the opening word is still not treated as an entity`() {
        assertEquals(setOf("docker"), dev.nacode.kmemo.guard.Text.entityTokens("What is Docker?"))
    ...
    fun `a miss reports the reason and the score of the same candidate`() = runTest {
        ...
        val cache = SemanticCache(
            embedder = dev.nacode.kmemo.fixtures.FixedEmbedder(
                mapOf(
