...
import dev.kmemo.fixtures.UpstreamFailure
...
class ResilienceTest {
    ...
    fun `by default an embed failure propagates out of getOrPut`() = runTest {
        ...
        assertFailsWith<UpstreamFailure> {
    ...
    fun `lookup and put propagate an embed failure regardless of policy`() = runTest {
        ...
        assertFailsWith<UpstreamFailure> { cache.lookup("anything") }
        assertFailsWith<UpstreamFailure> { cache.put("anything", "answer") }
    }
    ...
    fun `a fall-back emits Degraded naming the operation the policy and the cause`() = runTest {
        ...
        assertIs<UpstreamFailure>(degraded.cause)
    }
    ...
    fun `nothing is reported under PROPAGATE where the caller sees the exception instead`() = runTest {
        ...
        assertFailsWith<UpstreamFailure> { cache.getOrPut("anything") { "unreachable" } }
    ...
    fun `a retrying embedder gives up after maxAttempts and surfaces the failure`() = runTest {
        ...
        assertFailsWith<UpstreamFailure> { embedder.embed("hello") }
        assertEquals(3, flaky.attempts)
    }
    ...
    fun `retryOn can rule a failure not worth retrying`() = runTest {
        val flaky = FlakyEmbedder(failures = 99)
        val embedder = flaky.retrying(maxAttempts = 5, initialDelay = 1.milliseconds) { it !is UpstreamFailure }
    ...
    private class AlwaysFailingEmbedder : Embedder {
        override suspend fun embed(text: String): FloatArray = throw UpstreamFailure("embedder is down")
    }
    ...
    private class FlakyEmbedder(
        ...
        override suspend fun embed(text: String): FloatArray {
            attempts++
            if (attempts <= failures) throw UpstreamFailure("transient failure #$attempts")
