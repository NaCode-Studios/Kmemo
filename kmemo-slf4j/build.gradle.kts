...
dependencies {
    ...
    // A real SLF4J backend so tests can attach a ListAppender and assert on captured events.
...
mavenPublishing {
    ...
    coordinates("io.github.nacode-studios", "kmemo-slf4j", version.toString())
    pom {
        name.set("Kmemo SLF4J logging")
        description.set(
            "A JVM-only SLF4J CacheListener for Kmemo, the Kotlin Multiplatform semantic cache for LLM " +
                "calls — a structured log line per hit, miss, write and eviction, with prompt redaction " +
                "on by default.",
        )
